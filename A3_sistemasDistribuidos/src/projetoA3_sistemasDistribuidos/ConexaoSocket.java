...
public class ConexaoSocket extends Thread {
    ...
    public void run() {
        try {
            ...
            // Modificação para tratar escolhas inválidas
            while (!escolhaModo.equals("1") && !escolhaModo.equals("2")) {
                escritor.println("Escolha inválida. Escolha outra opção \n 1 - Robo (CPU) \n 2 - Outro Jogador. \n Faça sua escolha:");
                escolhaModo = leitor.readLine();
                System.out.println("Escolha do cliente: " + escolhaModo); // Depuração
            }
    ...
    private void jogarContraRobo() throws IOException {
        ...
        while (rodada < totalRodadas) {
            ...
            System.out.println("Escolha do jogador: " + escolhaJogador); // Depuração

            // Tratar escolhas inválidas
            while (escolhaJogador == null ||
                   escolhaJogador.isEmpty() ||
                   (!escolhaJogador.equalsIgnoreCase("pedra") &&
                    !escolhaJogador.equalsIgnoreCase("papel") &&
                    !escolhaJogador.equalsIgnoreCase("tesoura"))) {
                escritor.println("Escolha inválida. Escolha novamente entre pedra, papel ou tesoura. \n Faça sua escolha:");
                escolhaJogador = leitor.readLine();
                System.out.println("Nova escolha do jogador: " + escolhaJogador); // Depuração
            }
            ...
            if (escolhaJogador.equalsIgnoreCase(escolhaRobo)) {
                escritor.println("Empate!");
                estatisticas[2]++;
            } else if ((escolhaJogador.equalsIgnoreCase("pedra") && escolhaRobo.equals("tesoura")) ||
                       (escolhaJogador.equalsIgnoreCase("papel") && escolhaRobo.equals("pedra")) ||
                       (escolhaJogador.equalsIgnoreCase("tesoura") && escolhaRobo.equals("papel"))) {
        ...
        // Perguntar se o jogador deseja jogar novamente ou sair
        while (true) {
            escritor.println("Deseja jogar novamente ou sair? \n Digite 'novamente' para jogar novamente ou 'sair' para encerrar.\n Faça sua escolha:");
            String escolhaFinal = leitor.readLine();
            if (escolhaFinal.equalsIgnoreCase("novamente")) {
                run();
                break;
            } else if (escolhaFinal.equalsIgnoreCase("sair")) {
                escritor.println("Obrigado por jogar! Até a próxima.");
                break;
            } else {
                escritor.println("Escolha inválida, tente novamente.");
            }
        }
    }
    ...
    private void finalizarJogo() {
        ...
        primeiroCliente.escritor.println("O Jogo está sendo finalizado, obrigado por jogar com nós");
        segundoCliente.escritor.println("Estatísticas da partida - Vitórias: " + segundoCliente.estatisticas[0] + ", Derrotas: " + segundoCliente.estatisticas[1] + ", Empates: " + segundoCliente.estatisticas[2]);
         segundoCliente.escritor.println("O Jogo está sendo finalizado, obrigado por jogar com nós");
    }

    private void RespostaJogadores() throws InterruptedException, IOException {
        while (rodada < totalRodadas) {
            String escolha = leitor.readLine();
            while (!escolha.equalsIgnoreCase("pedra") &&
